...
import { collection, onSnapshot, addDoc, doc } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
onSnapshot(doc(db, "settings", "general"), (docSnap) => {
  if (docSnap.exists()) {
    ...
    if(data.whatsapp) document.querySelector('.header-whatsapp').href = `https://wa.me/${data.whatsapp}`;
  }
});
...
onSnapshot(collection(db, "specialists"), (snapshot) => {
  ...
  wizardSpecialists.innerHTML = specialists.map(s => `
    <div class="option-card" data-value="${s.name}">
      <div class="option-info" style="display: flex; align-items: center; gap: 10px;">
        ...
        <div>
          <h4>${s.name}</h4>
          <span style="color: #ff9800; font-size: 0.8em;">★ ${s.rating}</span>
        </div>
      </div>
    </div>
  `).join('');
